...
IF EXISTS (SELECT TOP 1 1 
           FROM   [dbo].[Attendance])
    BEGIN
        INSERT INTO [dbo].[tmp_ms_xx_Attendance] ([studentName], [currentDate], [isPresent])
        SELECT   [studentName],
                 [currentDate],
                 [isPresent]
        FROM     [dbo].[Attendance]
        ORDER BY [studentName] ASC, [currentDate] ASC;
    END
...
DECLARE tableconstraintnames CURSOR LOCAL FORWARD_ONLY
    FOR SELECT SCHEMA_NAME([schema_id]),
               ...
               0
        FROM   [sys].[objects]
        WHERE  [parent_object_id] IN (OBJECT_ID(N'dbo.ForumTable'))
               AND [type] IN (N'F', N'C')
